...
/**
* FUNCTION: gammainc( arr, a, path[, sep[, tail[, regularized] ] ] )
...
*/
function gammainc( x, a, path, sep, tail, regularized ) {
	var len = x.length,
		opts = {},
		dget,
		dset,
	...
	if ( len ) {
		...
		if ( isTypedArrayLike( a ) ) {
			for ( i = 0; i < len; i++ ) {
				v = dget( x[ i ] );
				if ( typeof v === 'number' ) {
					dset( x[ i ], FUN( v, a[ i ], regularized ) );
				} else {
					dset( x[ i ], NaN );
				}
			}
