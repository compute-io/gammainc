...
function gammainc( x, y, options ) {
	...
	if ( isNumber( x ) || isnan( x ) ) {
		for ( var key in options ) {
			if ( key !== 'dtype' ){
				throw new Error( 'gammainc()::only dtype option is applicable when first argument is not array- or matrix-like. Keys: `' + Object.keys( options ) + '`.' );
			}
		}
		if ( isMatrixLike( y ) ) {
	...
	if ( isTypedArrayLike( x ) ) {
		...
		return gammainc5( out, x, y );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return gammainc3( x, y, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return gammainc2( out, x, y, opts.accessor );
		}
		return gammainc1( out, x, y );
	}
	return NaN;
