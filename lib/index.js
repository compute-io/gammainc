...
* @param {String} [opts.tail="lower"] - wheter to compute the lower or upper incomplete gamma function
...
function gammainc( x, y, options ) {
	...
	if ( isNumber( x ) || isnan( x ) ) {
		for ( var key in options ) {
			if ( key !== 'dtype' && key !== 'tail' ){
				throw new Error( 'gammainc()::only dtype and tail options are applicable when first argument is not array- or matrix-like. Keys: `' + Object.keys( options ) + '`.' );
			}
		}
		if ( isMatrixLike( y ) ) {
	...
	if ( scalarCase === true ) {
		return (opts.tail === 'upper') ? gammainc6.upper( x, y ) : gammainc6.lower( x, y );
	}
	if ( isMatrixLike( x ) ) {
		if ( opts.copy !== false ) {
		...
		return gammainc4( out, x, y, opts.tail );
	}
	if ( isTypedArrayLike( x ) ) {
		...
		return gammainc5( out, x, y, opts.tail );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return gammainc3( x, y, opts.path, opts.sep, opts.tail );
		}
		...
		if ( opts.accessor ) {
			return gammainc2( out, x, y, opts.accessor, opts.tail );
		}
		return gammainc1( out, x, y, opts.tail );
	}
	return NaN;
